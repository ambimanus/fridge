...
public class TimedControllerCompactLinear extends BaseControllerCompactLinear
	...
	public void doReduceLoad(Double tau_preload, Double tau_reduce) {
		...
		double T_max_act = CompactLinearFridge.calculateTemperatureAfter(
				tau_T_min_to_T_max_act, fridge.getT_min(), fridge, fridge
						.getQ_warming());
		...
		double T_max_not = CompactLinearFridge.calculateTemperatureAfter(
				tau_T_max_to_T_max_not, fridge.getT_max(), fridge, fridge
						.getQ_cooling());
		// Maximal temperature at T_notify to reach T_min after tau_preload
		double T_mid_not = CompactLinearFridge.calculateTemperatureAfter(
				tau_cooling - tau_preload, fridge.getT_max(), fridge, fridge
						.getQ_cooling());
		// Maximal temperature at T_notify to survive tau_reduce without preload
		double T_min_not = CompactLinearFridge.calculateTemperatureAfter(
				tau_T_min_to_T_max_act - tau_preload, fridge.getT_min(),
				fridge, fridge.getQ_warming());
